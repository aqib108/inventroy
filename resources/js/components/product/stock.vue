<template>
  ...
  <tbody>
    <tr v-for="(product,idx) in filtersearch" :key="idx">
      ...
      <td>{{product.qty}}</td>
      <td>
      <span v-if="product.qty>=1" class="badge badge-success" >Avilable</span>
      ...
      <td><img :src="product.image" style="height: 90px; width: 80px" /></td>
        <td>
         ...
         <button class="btn btn-xs btn-outline-success"><router-link :to="{ name:'Edit-Stock', params:{product_id:product.id } }"><i class="fa fa-edit"></i>Stock</router-link></button>
        </td>
    </tr>
